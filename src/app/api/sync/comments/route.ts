...
import { syncCommentsToFarcaster } from '@/utils/farcasterSync';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const syncResult = await syncCommentsToFarcaster(hubbleUrl);
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      hubbleInfo,
      usage: {
        POST: "Submit a POST request to this endpoint to trigger comment syncing",
        body: {
