...
import { syncCastsToFarcaster } from '@/utils/farcasterSync';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const syncResult = await syncCastsToFarcaster(hubbleUrl);
    
    return NextResponse.json(syncResult);
  } catch (error) {
    console.error('Error syncing casts:', error);
    return NextResponse.json({ 
      success: false, 
      error: 'Failed to sync casts',
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      hubbleInfo,
      usage: {
        POST: "Submit a POST request to this endpoint to trigger cast syncing",
        body: {
